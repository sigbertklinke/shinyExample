...
#'
#' Constructs a slider widget to select a confidence level. For the not documented parameters see \code{\link[shiny]{sliderInput}}
#'
#' @param inputId 
...
confidenceLevelInput <- function (inputId, label, min, max, value, step, round = FALSE,
  ...
  if (percent) {
    ...
    ret <- sliderInput(inputId, label, min, max, value, step, round , format, locale, ticks, animate,
