...
axis( side=1, at=c(0, 15, 20, 60), labels=c(expression(theta[0]), "C", expression(theta[1]), "V"), 
      tick=FALSE)
axis( side=2, at=c(0.039), labels=c("f(V)"), tick=FALSE)

legend("topright", lwd=4, col=c("red","blue"),c("Verteilung unter H0", "Verteilung unter H1"), bty=
         "n")
...
axis( side=1, at=c(0, 18, 23, 60), labels=c(expression(theta[0]), "C", expression(theta[1]), "V"), 
      tick=FALSE)
axis( side=2, at=c(0.039), labels=c("f(V)"), tick=FALSE)
